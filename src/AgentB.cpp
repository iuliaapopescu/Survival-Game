...
int AgentB::getPosition(vector <Agent*> agents){
        int position = -1;
...
void AgentB::takeItem(char **map, vector<Item *> &items, int x, int y) {
    if(!isalpha(map[x][y]) && map[x][y] != '.') {
        for (Item *temp : items)
            if (temp->getSymbol() == map[x][y]) {
                map[x][y] = symbol;
                map[AgentB::x][AgentB::y] = '.';
                AgentB::x = x; AgentB::y = y;
...
void AgentB::availablePosition(char **map, int x, int y) {
        if(map[x][y] == '.') {
            cout << "Agent " << getSymbol() << " moved from position " << AgentB::x << ' ' << AgentB::y << " to position " << x << " " << y << endl;
            map[x][y] = symbol;
            map[AgentB::x][AgentB::y] = '.';
            AgentB::x = x;
            AgentB::y = y;
...
char AgentB::getSymbol() const {
    return symbol;
}

int AgentB::getLife() const {
    return life;
}

void AgentB::setLife(int life) {
    AgentB::life = life;
}

int AgentB::getPower() const {
    return power;
}

void AgentB::setPower(int power) {
    AgentB::power = power;
}

int AgentB::getX() const {
    return x;
}

void AgentB::setX(int x) {
    AgentB::x = x;
}

int AgentB::getY() const {
    return y;
}

void AgentB::setY(int y) {
    AgentB::y = y;
}

void AgentB::attack(char **map, vector <Agent*> &agents, int newX, int newY) {
...
void AgentB::move(vector <Agent*> &agents, vector <Item*> &items, char **map, int dimension){
