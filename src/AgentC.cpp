...
int AgentC::getPosition(vector <Agent*> agents){
        int position = -1;
...
void AgentC::takeItem(char **map, vector<Item *> &items, int x, int y) {
    if(!isalpha(map[x][y]) && map[x][y] != '.') {
        for (Item *temp : items)
            if (temp->getSymbol() == map[x][y]) {
                map[x][y] = symbol;
                map[AgentC::x][AgentC::y] = '.';
                AgentC::x = x;
                AgentC::y = y;
...
void AgentC::availablePosition(char **map, int x, int y) {
        if(map[x][y] == '.') {
            cout << "Agent " << getSymbol() << " moved from position " << AgentC::x << ' ' << AgentC::y << " to position " << x << " " << y << endl;
            map[x][y] = symbol;
            map[AgentC::x][AgentC::y] = '.';
            AgentC::x = x; AgentC::y = y;
        }
    }
...
char AgentC::getSymbol() const {
    return symbol;
}

int AgentC::getLife() const {
    return life;
}

void AgentC::setLife(int life) {
    AgentC::life = life;
}

int AgentC::getPower() const {
    return power;
}

void AgentC::setPower(int power) {
    AgentC::power = power;
}

int AgentC::getX() const {
    return x;
}

void AgentC::setX(int x) {
    AgentC::x = x;
}

int AgentC::getY() const {
    return y;
}

void AgentC::setY(int y) {
    AgentC::y = y;
}

void AgentC::attack(char **map, vector <Agent*> &agents, int newX, int newY) {
...
void AgentC::move(vector <Agent*> &agents, vector <Item*> &items, char **map, int dimension){
